...
any( !is.finite( denguedat_complete$CASE_FIRST_CDATE ) )
any( is.na( denguedat_complete$ODATE ) )
any( is.na( denguedat_complete$CASE_FIRST_CDATE ) )

# denv_nowcast <- NobBS(data=denguedat_complete, now = as.Date("1990-01-01"),
#                       units = "1 day", onset_date = "ODATE",
#                       report_date = "CASE_FIRST_CDATE")
denguedat_complete_df <- data.frame(denguedat_complete)

denguedat_complete_df$onset_week <- as.Date(NA)
